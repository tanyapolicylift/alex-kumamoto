...
To prevent unauthorized access, detect external security threats and to keep Company's network separate from unsecured networks, the Internet, and third-party networks, PolicyLift uses firewalls and intrusion detection capabilities (such as cloud-native detection services).
...
All remote access to PolicyLift systems must occur through encrypted and authenticated channels (e.g., SSO with MFA for SaaS applications, SSH with key-based authentication for infrastructure, or VPN where appropriate). Management limits access to remote administration tools to authorized individuals.
...
Every access to the network and network security products by administrators should be permitted, tracked, and overseen. The IT/Engineering team of PolicyLift should monitor all network services and their use. Alerting should be configured to detect suspicious network activity or potential intrusions.
...
## **Save Network Device Setup**
 ...
 Infrastructure configurations are defined in code (infrastructure-as-code) and stored in version control, or backed up to a secure central location when IaC is not applicable.
...
### **Building and Sustaining Skills**
 ...
 Engineering staff responsible for infrastructure are expected to maintain relevant technical skills. Network security topics are included in the company's broader security awareness program.
...
### **Mobile Device Management**
 ...
 All endpoint devices used to access PolicyLift systems must meet security baselines, including disk encryption, screen auto-lock, and current operating system patches. Compliance with these baselines is verified periodically.
...
Devices need to be set up with a secure password in line with PolicyLift's password policy. This password should be unique and not be the same as any other used within the organization. Users agree not to share their passwords with anyone and must auto-lock with a password or PIN after a period of inactivity.
 ...
 Only devices that meet PolicyLift's security baselines may access corporate systems. Devices not in compliance with security requirements will not be granted access.

 Security requirements including encryption, password policies, and screen lock are enforced through documented baselines and periodic verification.

 Any attempt to circumvent security controls on endpoint devices will result in immediate disconnection from corporate resources, and additional actions may follow according to PolicyLift's security policy.

 All remote connections to corporate systems must use encrypted channels.
