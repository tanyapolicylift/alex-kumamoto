...
Please contact the appropriate internal contact if you have any questions about PolicyLift information security program.
...
The Chief Technology Officer (CTO) has the authority to modify, remove, or approve modifications to these policies and procedures.

 Records of all such changes must be kept.
...
PolicyLift promotes the understanding of secure coding to its engineers in order to improve the security and robustness of PolicyLift products. Security awareness training is provided to all personnel upon hire and at least annually. Completion of training is tracked and documented.
...
- Workstations should be configured to receive automatic patch updates. Patch compliance is monitored on an ongoing basis through endpoint management tooling.

- Devices are securely wiped before reuse or disposal.
## Data Management
...
A patch management process exists to confirm that operating system level vulnerabilities must be remediated in a timely manner. In addition, production servers must be scanned to test patch compliance on an ongoing basis.
 ...
 Engineering leadership reviews and approves critical patches on a timely basis.
...
PolicyLift collects & monitors audit logs and alerts on key events stemming from production systems, applications, databases, servers, and critical services, as well as IAM user and admin activities. IT team continuously monitors system capacity and performance through the use of monitoring tools to identify and detect anomalies that could compromise availability of the system operations.
...
A message should be sent to the appropriate internal security contact if you believe there may be a security incident or threat.
