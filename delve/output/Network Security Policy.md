<!--
  REDLINE DOCUMENT — Network Security Policy
...
To prevent unauthorized access, detect external security threats and to keep Company's network separate from unsecured networks, the Internet, and third-party networks, PolicyLift uses firewalls and ~~intrusion detection systems~~ **[INSERT: intrusion detection capabilities (such as cloud-native detection services)]**.
...
All remote access to PolicyLift systems must occur through ~~a sanctioned medium, like a VPN, based on thorough risk assessment and proper authorizations~~ **[INSERT: encrypted and authenticated channels (e.g., SSO with MFA for SaaS applications, SSH with key-based authentication for infrastructure, or VPN where appropriate)]**. ~~Also, utilize VPN connections over public networks for encrypting sensitive information and management limits VPN access to authorized individuals. Administrator access to the VPN console is restricted to authorized individuals.~~ **[INSERT: Management limits access to remote administration tools to authorized individuals.]**
...
Every access to the network and network security products by administrators should be permitted, tracked, and overseen. The IT/Engineering team of PolicyLift should monitor all network services and their use. ~~All traffic passing through the firewall should be observed for potential misuses and intrusions.~~ **[INSERT: Alerting should be configured to detect suspicious network activity or potential intrusions.]**
...
~~The setup of network devices should be saved regularly or anytime changes are made, and this should be done to a central location. This central backup location needs to be kept secure.~~ **[INSERT: Infrastructure configurations are defined in code (infrastructure-as-code) and stored in version control, or backed up to a secure central location when IaC is not applicable.]**
...
~~Guarantee that PolicyLift's network support team has the necessary training and internal communication to establish and maintain a secure network infrastructure.~~ **[INSERT: Engineering staff responsible for infrastructure are expected to maintain relevant technical skills. Network security topics are included in the company's broader security awareness program.]**
...
~~The company has a mobile device management (MDM) system in place to centrally manage endpoint devices supporting the service to the customer. The MDM tool is appropriately configured to prevent against unauthorized or inappropriate connectivity of endpoint devices to PolicyLift's environment.~~ **[INSERT: All endpoint devices used to access PolicyLift systems must meet security baselines, including disk encryption, screen auto-lock, and current operating system patches. Compliance with these baselines is verified periodically.]**
...
~~Only IT-managed or IT-approved devices can directly connect to the internal corporate network and PolicyLift's IT department will inspect all personal mobile devices attempting to connect to the corporate network via the internet. Devices that are not IT-supported, not in compliance with IT's security policies, or pose any threat to the corporate network or data will not be granted access.~~ **[INSERT: Only devices that meet PolicyLift's security baselines may access corporate systems. Devices not in compliance with security requirements will not be granted access.]**

~~Such devices shall comply with the existing rules on security features like encryption, password, key lock, etc. The IT department will enforce these policies using Mobile Device Management (MDM) software whenever possible.~~ **[INSERT: Security requirements including encryption, password policies, and screen lock are enforced through documented baselines and periodic verification.]**

~~Any attempt to undermine or bypass the MDM system will result in immediate disconnection from all corporate resources, and additional actions may follow according to PolicyLift's comprehensive security policy.~~ **[INSERT: Any attempt to circumvent security controls on endpoint devices will result in immediate disconnection from corporate resources, and additional actions may follow according to PolicyLift's security policy.]**

~~Devices may only connect to the corporate network and data over the internet via a Secure Socket Layer (SSL) Virtual Private Network (VPN).~~ **[INSERT: All remote connections to corporate systems must use encrypted channels.]**
