...
type: text
note: "2-step closing flow. Always start with confirmed=false."
---
...
2. After Step 1 has already happened in this conversation, call hangUp(confirmed=true)
   only if the user clearly indicates they do not need anything else and still want to end the call.
...
- The assistant asking a similar question manually does not replace Step 1.
- The user's initial "bye", "that's all", or "hang up" should still start with confirmed=false.
...
- When unsure, prefer confirmed=false as the safe default.
