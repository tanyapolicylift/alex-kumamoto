...
Insurance agencies serving Spanish-speaking clients need quote packet forms rendered in Spanish. All template labels, help text, descriptions, and select option labels were English-only.

### Decision

Add a nullable JSONB column `multi_language` to three template tables (`entity_templates`, `field_group_templates`, `field_templates`). Translations are stored as a nested object keyed by language code:

```jsonb
{
  "es": {
    "label": "Información del solicitante",
    "description": "Detalles del solicitante principal"
  }
}
```

For select/multi_select field options, the `es` key is added directly to each option object in the existing `options` JSONB column:

```json
[
  { "label": "Yes", "value": "yes", "es": "Sí" },
  { "label": "No", "value": "no", "es": "No" }
]
```

### Feature Flag

Multi-language support is gated behind a PostHog feature flag `quote-packet-multilingual`, evaluated per agency. When disabled, the admin UI hides Spanish input fields and the public form hides the language switcher.

### Public Form Routing

Language is selected via URL path segment: `/forms/[slug]/es` for Spanish, `/forms/[slug]` for English (default). A catch-all route `[[...lang]]` handles both patterns. When the flag is enabled, an EN/ES switcher appears in the form header.

### Translation Fallback

All labels fall back to the English value when a translation is missing for the requested language. This ensures forms always render correctly even with partial translations.

### Admin UI

When `quote-packet-multilingual` is enabled for an agency, entity, field group, and field template form dialogs show additional Spanish input fields (label, description/help text). These are optional — templates work in English-only without filling them in.

### API

The smart form endpoint accepts an optional `?lang=es` query parameter. When present, the API returns localized labels in the JSON schema `description` fields, falling back to English when a translation is not available.
