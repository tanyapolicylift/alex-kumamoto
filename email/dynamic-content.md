...
> **Verified precedent — Agency Revolution works this way (2026-06-01).** AR runs sequences on a **per-entity basis** — per-account / per-policy / per-contact / per-claim (its four entity bases) — and that basis *gates which merge fields are available*: to merge `{{policy.*}}` the sequence must enroll **per-policy**, so each enrollment binds to exactly one record. Per AR's docs, a campaign can't expose claim merge fields unless it enrolls on the New Claim Event, *"since it is possible for customers to have more than one claim over time, the system will not know which claim it is basing the campaign on."* That's Path B enforced at the merge-field layer — re-anchor so there's never more than one record in context. AR's ceiling is the *enumerate-a-list* case: it doesn't loop a matched set, it ships a dedicated **active-policies component** (a fixed widget; teardown `research_feature_list.md` 1.1.8) and does conditional show/hide via Segment Match at the sequence level (1.1.18). So AR proves Path B ships, and shows exactly where it runs out — which is the gap Path A (a general matched-set loop) would beat.
...
> **Not how Agency Revolution does it (verified 2026-06-01).** An earlier guess was that AR might expose a render-time data pull of this kind. It doesn't. AR's mechanism is **per-entity sequence anchoring (Path B above) plus a fixed active-policies component** — content is gated by the enrollment's entity basis, never pulled by a query inside the template. So there is **no insurance-industry precedent for Path C**; the Salesforce Marketing Cloud `LookupRows` model below remains the only reference for it.
...
3. **AR mechanism — RESOLVED (2026-06-01).** AR does *not* use Path C render-time lookup. It uses **per-entity sequence anchoring (Path B)**: the enrollment's entity basis (one of account / policy / contact / claim) gates merge-field availability, so the campaign always has exactly one record in context; lists are handled by a fixed **active-policies component**, not a matched-set loop. AR is the **verified precedent for Path B**; there is no insurance-industry precedent for Path C. (Sources: AR help docs on merge-field availability + advanced segments; teardown `research_feature_list.md` 1.1.8 / 1.1.18 / 1.1.14. See Path B / Path C notes above.)
